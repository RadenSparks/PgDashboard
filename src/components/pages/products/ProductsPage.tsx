...
const ProductsPage = () => {
    ...
    const handleSaveNewProduct = async () => {
        try {
            toaster.show({ title: "Đang lưu sản phẩm...", type: "loading" });
            if (editProduct) {
                ...
                toaster.show({ title: "Đã lưu sản phẩm thành công!", type: "success" });
                setEditProduct(null);
                setShowAddModal(false);
            }
        } catch (err: unknown) {
            const errorMsg =
                typeof err === "object" && err !== null && "message" in err
                    ...
                    : "Unknown error";
            toaster.show({ title: "Có lỗi khi lưu sản phẩm", description: errorMsg, type: "error" });
        }
    };
    ...
    const handleSaveEditProduct = async () => {
        try {
            toaster.show({ title: "Đang cập nhật sản phẩm...", type: "loading" });
            if (editProduct) {
                const formData = new FormData();
                ...
                toaster.show({ title: "Đã cập nhật sản phẩm thành công!", type: "success" });
                setEditProduct(null);
                setShowAddModal(false);
            }
        } catch (err: unknown) {
            const errorMsg =
                typeof err === "object" && err !== null && "message" in err
                    ? String((err as { message: string }).message)
                    : "Unknown error";
            toaster.show({ title: "Có lỗi khi cập nhật sản phẩm", description: errorMsg, type: "error" });
        }
    };
    ...
    const handleDelete = async (id: number) => {
        if (window.confirm("Bạn có chắc chắn muốn xóa sản phẩm này? Hành động này không thể hoàn tác.")) {
            toaster.show({ title: "Đang xóa sản phẩm...", type: "loading" });
            await deleteProduct(id);
            toaster.show({ title: "Đã xóa sản phẩm.", type: "success" });
        }
    };
        ...
        <div className="p-8">
            ...
            <div className="flex flex-wrap gap-6 mb-10 items-end bg-white rounded-xl shadow-sm px-6 py-5 border border-gray-100">
                ...
                <div className="flex flex-col min-w-[160px]">
                    <label className="block text-xs font-semibold text-blue-700 mb-1 ml-1 tracking-wide">Danh mục</label>
                    <select
                ...
                <div className="flex flex-col min-w-[140px]">
                    <label className="block text-xs font-semibold text-blue-700 mb-1 ml-1 tracking-wide">Thể loại</label>
                    <select
                        className="border border-blue-300 focus:border-blue-500 focus:ring-2 focus:ring-blue-200 rounded-lg px-3 py-2 bg-blue-50 text-blue-900 font-medium shadow-sm transition outline-none"
                        value={genreFilter}
                ...
                <div className="flex flex-col min-w-[120px]">
                    <label className="block text-xs font-semibold text-blue-700 mb-1 ml-1 tracking-wide">Số người chơi</label>
                    <select
                        className="border border-blue-300 focus:border-blue-500 focus:ring-2 focus:ring-blue-200 rounded-lg px-3 py-2 bg-blue-50 text-blue-900 font-medium shadow-sm transition outline-none"
                        value={playersFilter}
                ...
                {/* Duration Dropdown */}
                <div className="flex flex-col min-w-[120px]">
                    <label className="block text-xs font-semibold text-blue-700 mb-1 ml-1 tracking-wide">Thời lượng</label>
                    <select
                        className="border border-blue-300 focus:border-blue-500 focus:ring-2 focus:ring-blue-200 rounded-lg px-3 py-2 bg-blue-50 text-blue-900 font-medium shadow-sm transition outline-none"
                ...
                {/* Age Dropdown */}
                <div className="flex flex-col min-w-[120px]">
                    <label className="block text-xs font-semibold text-blue-700 mb-1 ml-1 tracking-wide">Độ tuổi</label>
                    <select
                        className="border border-blue-300 focus:border-blue-500 focus:ring-2 focus:ring-blue-200 rounded-lg px-3 py-2 bg-blue-50 text-blue-900 font-medium shadow-sm transition outline-none"
                        value={ageFilter}
                ...
                <div className="flex flex-col flex-1 min-w-[220px]">
                    <label className="block text-xs font-semibold text-blue-700 mb-1 ml-1 tracking-wide">Tìm kiếm</label>
