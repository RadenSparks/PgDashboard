            ...
            <table className="min-w-full text-sm align-middle">
                ...
                <tbody>
                    {products.map((prod) => {
                        // Group tags by type
                        const genres =
                            prod.tags?.filter((t) => genreTags.some((g) => g.genre === t)) || [];
                        const players =
                            prod.tags?.find((t) => playerTags.some((p) => p.players === t)) || "";
                        const duration =
                            prod.tags?.find((t) => durationTags.some((d) => d.duration === t)) || "";
                        return (
                            <tr key={prod.id} className="border-b hover:bg-gray-50 align-middle">
                                ...
                                <td className="py-3 px-2 font-semibold align-middle">{prod.name}</td>
                                <td className="py-3 px-2 max-w-xs align-middle">
                                    <span title={prod.description} className="block truncate">
                                        {prod.description}
                                    </span>
                                </td>
                                <td className="py-3 px-2 align-middle">{prod.category}</td>
                                <td className="py-3 px-2 align-middle">
                                    {genres.length > 0 ? (
                                ...
                                <td className="py-3 px-2 align-middle">${prod.price.toFixed(2)}</td>
                                <td className="py-3 px-2 align-middle">
                                    {prod.discount > 0 ? (
                                ...
                                <td className="py-3 px-2 align-middle">{prod.stock}</td>
                                <td className="py-3 px-2 align-middle">{prod.sold}</td>
                                <td className="py-3 px-2 align-middle">
                                    <span
                                        className={
